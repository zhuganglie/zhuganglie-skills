...
6. 能并行就并行：分析长篇时按 segment 并行，生成阶段按 batch 并行，验证和重试串行。建议执行多步任务时使用 `todowrite` 工具来追踪和管理进度。
7. 任一子步骤返回非 JSON 时，应首先尝试提取 markdown 代码块（如 ` ```json ... ``` `）中的内容。若依然解析失败，再重跑该子步骤一次，并再次强调“只输出 JSON”。
...
```json
{
  ...
  "frames": [
    {
      "frame_number": 1,
      "plot_marker": "开场", // 必须且仅限使用 analyzer 允许的 6 种标志
      "scene": {
        "time": "白天",
        "location": "地点",
        "atmosphere": "氛围"
...
- 使用 `task` 工具（指定 `subagent_type: "general"`）为每个 batch 启动独立子代理并行生成
- 确保将替换好占位符的完整模板内容作为 `prompt` 参数传给 `task` 工具
- 不要等一个 batch 完成后再启动下一个，同时发出所有 task 调用
...
| 问题 | 处理 |
|---|---|
| 分析返回非 JSON | 尝试提取 ```json 代码块内容，失败则重跑分析强调“只输出 JSON” |
